...
export class CinemaDetailsComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.cinemaService.getYelpCinemaDetails(this. cinemaId).subscribe((response) => {
  ...
  this.cinemaService.getYelpCinemaReviews(this. cinemaId).subscribe((response) => {
      this.cinemaReviews = response.reviews;
  });
}
}