...
markers = [];
  ...
  constructor(public auth: AuthService, public router: Router, public fb: FormBuilder, public festivalsService: FestivalsService, public meteo: MeteoService, private httpClient : HttpClient) { 
   ...
   var req = {
    ...
    this.festivalsService.getYelpFestivals(req).subscribe((response) => {
      this.festivalsList = response;
      console.log(this.festivalsList);
      this.markers.forEach(marker => {
        this.removeMarker(marker);
      });
      this.markers = [];
      this.festivalsList.forEach(festivals => {
        this.addMarker(festivals);
      })
      this.findBestZoom();
    })
 ...
 toggle() {
  ...
  addMarker(festivals) {
    var marker = L.marker([festivals.coordinates.latitude, festivals.coordinates.longitude], { icon: this.myIcon });
    marker.addTo(this.map);
    marker.bindPopup(festivals.name).openPopup();
  ...
  goToFestivalsDetails(festivals){
    this.router.navigateByUrl('/festivalsDetails?festivalsId='+festivals.id); 
  }
  ngOnInit() {
   ...
   // address autocomplete
    ...
    this.map.addLayer(this.mapboxLayer);
     ...
     if(localStorage.getItem('festivalsList')){
    this.festivalsList=JSON.parse(localStorage.getItem('festivalsList'));
     this.markers.forEach(marker => {
        this.removeMarker(marker);
      });
      this.markers = [];
    ...
    this.meteo.detectLocation(position => this.initFestivals(position));
}
  }
  }  